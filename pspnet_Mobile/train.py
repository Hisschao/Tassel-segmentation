...
def generate_arrays_from_file(lines,batch_size):
    ...
    while 1:
        ...
        for _ in range(batch_size):
            ...
            # 从文件中读取原图像
            img = gdal.Open(r"../mydatamul/tif" + '/' + name) ##00改
            ...
            # 从文件中读取mask图像
            img = gdal.Open(r"../mydatamul/png" + '/' + name)
            im_width = img.RasterXSize  # 栅格矩阵的列数
            im_height = img.RasterYSize  # 栅格矩阵的行数
            img = img.ReadAsArray(0, 0, im_width, im_height, None, int(WIDTH/4), int(HEIGHT/4))  # 获取数据
...
if __name__ == "__main__":
    ...
    # model.summary()
    # 打开数据集的txt
    with open(r"../mydatamul/train_data.txt","r") as f:
        lines = f.readlines()
    ...
    checkpoint_period = ModelCheckpoint(
                                    log_dir + 'mul_ep{epoch:03d}-loss{loss:.3f}-val_loss{val_loss:.3f}.h5',
                                    monitor='val_loss', 
